...
import argparse
import logging
from DS.data_set_joint import T5TitleDataSet, collate_func
...
def train(model, device, train_data, args):
    """
    训练模型
    Args:
        model: 模型
...
def set_args():
    ...
    parser.add_argument('--train_file_path', default='./data_dir/train_stage2_0.5.jsonl', type=str, help='新闻标题生成的训练数据')
    parser.add_argument('--test_file_path', default='./data_dir/test.jsonl', type=str, help='新闻标题生成的测试数据')
...
def main():
    ...
    # 加载模型的config
    print("加载模型的config")
    model = MT5PForSequenceClassification(args)
    model.to(device)
    ...
    # 加载训练数据和测试数据
    print("加载训练数据和测试数据")
    train_data = T5TitleDataSet(tokenizer, args.max_len, args.title_max_len, args.data_dir, "train",
